...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .qr-wrap { width: 24mm; height: 24mm; margin: 0.5mm auto; display:flex; align-items:center; justify-content:center; }
</style>
</head>
<body>
        ...
        <td class="lbl">
          ...
          <div style="margin-top:2mm;">
            @if(!empty($item->qrCodeBase64))
              <div class="qr-wrap">
                <img src="{{ $item->qrCodeBase64 }}" alt="QR Code" style="width:24mm; height:24mm; display:block;" />
              </div>
            @elseif(!empty($item->barcodeBase64))
              <img src="{{ $item->barcodeBase64 }}" alt="barcode" style="width:28mm; height:auto; display:block; margin:0.5mm auto;" />
            @elseif(!empty($item->barcodeSvgBase64))
              <img src="{{ $item->barcodeSvgBase64 }}" alt="barcode" style="width:28mm; height:auto; display:block; margin:0.5mm auto;" />
            @elseif(!empty($item->barcodeSvg))
              <div class="barcode-svg" style="display:block; margin:0.5mm auto;">{!! $item->barcodeSvg !!}</div>
            @elseif(!empty($item->barcodeHtml))
              <div class="barcode-html" style="display:block; margin:0.5mm auto;">{!! $item->barcodeHtml !!}</div>
            @endif
            <div style="font-size:5pt;color:#999;margin-top:1mm;text-align:center;">{{ !empty($item->qrCodeBase64) ? 'QR' : (!empty($item->barcodeBase64) ? 'PNG' : (!empty($item->barcodeSvg) ? 'SVG' : (!empty($item->barcodeHtml) ? 'HTML' : 'NONE'))) }}</div>
          </div>
          <hr class="hr">
